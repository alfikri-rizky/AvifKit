    ...
    Column(
      ...
      Column {
        SwitchWithLabel(
          ...
          onCheckedChange = {
            maxDimEnabled = it
            onParamsChange(params.copy(maxDimension = if (it) maxDimValue.toInt() else null))
          },
        )
      ...
      Column {
        ...
        if (maxSizeEnabled && currentFileSize != null) {
          ...
          if (maxSizeValue < 50 * 1024) { // Less than 50 KB
            Text(
              text =
                "⚠️ Very small target size. Result may not reach target due to AVIF format overhead and image complexity.",
              style = MaterialTheme.typography.bodySmall,
          ...
          Text(
            text =
              "Original size: ${formatFileSize(currentFileSize)} • The converter will try up to 10 attempts to reach the target by adjusting quality and dimensions.",
            style = MaterialTheme.typography.bodySmall,
            ...
            OutlinedButton(
              ...
              Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                  Text(
                    text =
                      when (params.compressionStrategy) {
              ...
              text =
                when (params.compressionStrategy) {
                  CompressionStrategy.SMART ->
                    "🎯 SMART: Finds the highest quality image that meets your target size using binary search. Faster (6-8 attempts) and produces better-looking images."
                  CompressionStrategy.STRICT ->
                    "🗜️ STRICT: Continues compressing even after meeting target to find the smallest possible size. May take longer (up to 10 attempts)."
                },
              style = MaterialTheme.typography.bodySmall,
              color = MaterialTheme.colorScheme.onSurfaceVariant,
              modifier = Modifier.padding(top = 4.dp),
...
private fun validateMaxSize(maxSize: Long, currentFileSize: Long?): String? {
  return when {
    ...
    currentFileSize != null && maxSize > currentFileSize ->
      "Max size cannot exceed original file size"
    else -> null
  }
}
