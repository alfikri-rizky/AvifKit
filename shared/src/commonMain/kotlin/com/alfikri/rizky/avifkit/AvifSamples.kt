...
/**
 * Sample usage examples for the AVIF Converter library These examples demonstrate common use cases
 */
object AvifSamples {
  ...
  suspend fun example2_CustomQuality(inputPath: String, outputPath: String): String {
    ...
    val customOptions = EncodingOptions(quality = 90, speed = 4, maxDimension = 2048)
  ...
  suspend fun example3_WithSizeLimit(inputPath: String, outputPath: String): String {
    ...
    val options =
      EncodingOptions.fromPriority(Priority.QUALITY)
        .copy(
  ...
  suspend fun example4_ConvertByteArray(imageData: ByteArray): ByteArray {
    ...
    return converter.encodeAvif(input = ImageInput.from(imageData), priority = Priority.BALANCED)
  }
  ...
  suspend fun example6_DecodeAvif(avifPath: String): PlatformBitmap {
    ...
    return converter.decodeAvif(input = ImageInput.from(avifPath))
  }
  ...
  suspend fun example7_GetImageInfo(imagePath: String): ImageInfo {
    ...
    return converter.getImageInfo(input = ImageInput.from(imagePath))
  }
  ...
  fun example8_CheckIfAvif(imagePath: String): Boolean {
    ...
    return converter.isAvifFile(input = ImageInput.from(imagePath))
  }
  ...
  suspend fun example10_BatchConversion(inputPaths: List<String>, outputDir: String): List<String> {
    val converter = AvifConverter()
  ...
  suspend fun example11_ProgressiveCompression(imagePath: String, targetSizeKB: Int): ByteArray {
    val converter = AvifConverter()
    ...
    for (quality in qualityLevels) {
      val result =
        converter.encodeAvif(
          input = ImageInput.from(imagePath),
          options = EncodingOptions(quality = quality, maxSize = targetSizeKB * 1024L),
        )
    ...
    return converter.encodeAvif(
      input = ImageInput.from(imagePath),
      options = EncodingOptions(quality = 40, maxDimension = 1024, maxSize = targetSizeKB * 1024L),
    )
  }

  /** Example 12: Safe conversion with error handling */
  suspend fun example12_SafeConversion(imagePath: String, outputPath: String): Result<String> {
    return try {
      val converter = AvifConverter()
